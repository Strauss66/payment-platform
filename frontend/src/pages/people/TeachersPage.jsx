...
import { listTeachers } from '../../lib/api.billing';
...
export default function TeachersPage(){
  ...
  async function load(){
    ...
    setState('loading');
    try {
      const { rows, count } = await listTeachers({ q: debouncedQ || undefined, department: department || undefined, limit, offset, sort: 'id:desc' });
      setRows(rows); setTotal(count ?? rows.length); setState('idle');
    } catch { setState('error'); }
  }

  useEffect(() => { load(); }, [currentSchoolId, debouncedQ, department, limit, offset]);
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap items-end gap-3 mb-3">
            <div>
              ...
              <input placeholder="Name, email…" value={q} onChange={e=>update('q', e.target.value)} className="border rounded px-3 py-2 w-64" />
            </div>
            <div>
              <label className="text-xs block mb-1">Department</label>
              <input placeholder="department" value={department} onChange={e=>update('department', e.target.value)} className="border rounded px-2 py-2 w-40" />
            </div>
            <div>
          ...
          {state === 'loading' ? (
            ...
            <Table head={<THead sticky columns={[{label:'Name'}, {label:'Role/Position'}, {label:'Department/Grade'}, {label:'Email'}, {label:'Phone'}, {label:'Actions'}]} />}>
              {rows.map((r) => (
                <TR key={r.id}>
...
function InlineError({ retry }){
  return (
    <div className="p-2 border border-red-200 bg-red-50 text-red-700 text-sm rounded">
      Failed to load teachers. <button onClick={retry} className="underline">Retry</button>
    </div>
  );
}
