...
import { listEmployees } from '../../lib/api.billing';
...
export default function EmployeesPage(){
  ...
  async function load(){
    ...
    setState('loading');
    try {
      const { rows, count } = await listEmployees({ q: debouncedQ || undefined, department: department || undefined, limit, offset, sort: 'id:desc' });
      ...
      <RoleGate allow={[ROLES.ADMIN, ROLES.SUPER_ADMIN]}>
        <div className="p-6">
          ...
          {state === 'loading' ? (
            ...
            <Table head={<THead sticky columns={[{label:'Name'}, {label:'Position'}, {label:'Department'}, {label:'Email'}, {label:'Phone'}, {label:'Actions'}]} />}>
              {rows.map((r) => (
                <TR key={r.id}>
...
function InlineError({ retry }){
  return (
    <div className="p-2 border border-red-200 bg-red-50 text-red-700 text-sm rounded">
      Failed to load employees. <button onClick={retry} className="underline">Retry</button>
    </div>
  );
}
