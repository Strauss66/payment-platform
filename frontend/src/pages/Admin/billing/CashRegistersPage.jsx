...
import { listCashRegisters, createCashRegister, updateCashRegister, deleteCashRegister } from '../../../lib/api.billing';
...
import CashRegisterForm from '../../../components/admin/billing/CashRegisterForm';
...
export default function CashRegistersPage() {
  ...
  const onDelete = async (row) => {
    if (!window.confirm('Delete this cash register?')) return;
    setState('deleting');
    try {
  ...
  const handleSubmit = async (payload) => {
    setState('saving');
    try {
      if (selected?.id) await updateCashRegister(selected.id, payload);
      else await createCashRegister(payload);
      ...
      <RoleGate roles={[ROLES.ADMIN, ROLES.SUPER_ADMIN]}>
        <div className="p-6">
          ...
          {state === 'loading' ? (
            ...
            <EmptyState title="No cash registers" description="Create your first cash register." />
          ) : (
          ...
          <Modal open={open} onClose={() => setOpen(false)} title={selected? 'Edit Cash Register' : 'New Cash Register'}>
            <CashRegisterForm initialValue={selected} onSubmit={handleSubmit} onCancel={() => setOpen(false)} submitting={state==='saving'} />
          </Modal>
        </div>
      </RoleGate>
    </ProtectedRoute>
