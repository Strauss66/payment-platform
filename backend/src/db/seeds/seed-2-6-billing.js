...
export default async function seedBilling() {
  ...
  try {
    ...
    const [[hs]] = await sequelize.query('SELECT id FROM levels WHERE school_id=? AND code="HS"', { replacements: [school.id], transaction: t });
    const [[sy]] = await sequelize.query('SELECT id, start_date, end_date FROM school_years WHERE school_id=? AND is_default=1', { replacements: [school.id], transaction: t });
    if (!hs || !sy) throw new Error('Missing HS level or default year');
    ...
    for (const [code, name, type, taxable] of concepts) {
      await sequelize.query(
        ...
        { replacements: [school.id, code, name, type, taxable], transaction: t }
      );
    }
    ...
    for (const [name, requiresRef] of pms) {
      await sequelize.query(
        ...
        { replacements: [school.id, name, requiresRef], transaction: t }
      );
    }
    ...
    await sequelize.query(
      ...
      { replacements: [school.id, hs.id], transaction: t }
    );
    ...
    const [[tuition]] = await sequelize.query('SELECT id FROM charge_concepts WHERE school_id=? AND code="TUITION"', { replacements: [school.id], transaction: t });
    const [students] = await sequelize.query('SELECT id FROM students WHERE school_id=?', { replacements: [school.id], transaction: t });
    ...
    for (const s of students) {
      for (const m of months) {
        await sequelize.query(
          ...
          { replacements: [school.id, s.id, tuition.id, m.month, m.year, m.year, String(m.month).padStart(2,'0')], transaction: t }
        );
      }
    }
